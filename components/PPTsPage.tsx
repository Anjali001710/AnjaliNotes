...
import { PresentationChartBarIcon, ChevronLeftIcon, PlusIcon, TrashIcon } from './Icons';
import type { Page, Subject } from '../App';

interface PPTsPageProps {
...
const PPTsPage: React.FC<PPTsPageProps> = ({ navigate, subjects, addSubject, deleteSubject, isAdmin }) => {
  const { t } = useLanguage();
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-8">
          {subjects.map(subject => (
            <SubjectCard
              key={subject.id}
              title={subject.name}
              onClick={() => navigate('ppts_subject', { subjectId: subject.id })}
