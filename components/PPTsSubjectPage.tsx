...
import { ChevronLeftIcon, PresentationChartBarIcon, PlusIcon, TrashIcon } from './Icons';
import type { Page, Subject, File } from '../App';

interface PPTsSubjectPageProps {
...
const PPTsSubjectPage: React.FC<PPTsSubjectPageProps> = ({ navigate, subject, files, addFile, deleteFile, isAdmin }) => {
  const { t } = useLanguage();
  ...
  if (!subject) {
    return (
      <div className="py-20 text-center">
        ...
        <button onClick={() => navigate('ppts')} className="text-blue-600 mt-4">Back to subjects</button>
      </div>
    );
  }
    ...
    <section className="py-20 bg-white min-h-full">
      <div className="container mx-auto px-6 max-w-4xl">
        <div className="text-center mb-12">
          ...
          <p className="text-lg text-gray-600 mt-2">{t('pptsFor')} {subject.name}</p>
        </div>
        ...
        <div className="space-y-4">
          {files.map((file) => (
             <div key={file.id} className="bg-gray-50 p-4 rounded-lg shadow-sm border border-gray-200 flex justify-between items-center">
              <div className="flex items-center">
                <PresentationChartBarIcon className="w-6 h-6 text-blue-500 mr-4" />
          ...
          {files.length === 0 && <p className="text-center text-gray-500 py-4">No PPTs found for this subject.</p>}
        </div>

        {isAdmin && <AddFileForm onAdd={addFile} />}

        <div className="text-center mt-12">
           <button onClick={() => navigate('ppts')} className="inline-flex items-center font-semibold text-blue-600 hover:text-blue-800 transition-colors">
...
export default PPTsSubjectPage;