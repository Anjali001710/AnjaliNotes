...
import { CodeBracketIcon, ChevronLeftIcon, PlusIcon, TrashIcon } from './Icons';
  ...
  return (
    <div className="relative group bg-white p-6 rounded-lg shadow-md border border-gray-200 flex flex-col items-center text-center hover:shadow-xl hover:-translate-y-1 transition-all duration-300">
      ...
      <button onClick={onClick} className="w-full h-full flex flex-col items-center">
        <CodeBracketIcon className="w-12 h-12 text-blue-500 mb-4" />
...
const ProgramsPage: React.FC<ProgramsPageProps> = ({ navigate, subjects, addSubject, deleteSubject, isAdmin }) => {
  const { t } = useLanguage();
    ...
    <section className="py-20 bg-gray-50 min-h-full">
      <div className="container mx-auto px-6">
        <div className="text-center mb-12">
          <h2 className="text-4xl font-bold text-gray-800">{t('programsSectionTitle')}</h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-8">
          {subjects.map(subject => (
            <SubjectCard
              key={subject.id}
              title={subject.name}
              onClick={() => navigate('programs_subject', { subjectId: subject.id })}
